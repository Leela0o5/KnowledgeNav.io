...
def test_valid_answer_passes(enforcer: CitationEnforcer) -> None:
    answer = (
        "The policy allows returns within 30 days [SOURCE:chunk1]. "
        "Refunds are processed in 5 business days [SOURCE:chunk2]."
    )
...
def test_low_coverage_fails(enforcer: CitationEnforcer) -> None:
    answer = (
        "The policy allows returns within 30 days. "
        "Also refunds take 5 days. Also shipping is free. Also taxes apply."
    )
    result = enforcer.validate(answer, {"chunk1"})
    assert not result.is_valid
